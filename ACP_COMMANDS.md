...
> **Prerequisite:** Build the uber-jar first with `mvn clean install`, then load the environment variables with `dotenv -x .env` (or `source .env` / `set -x` as appropriate for your shell).
